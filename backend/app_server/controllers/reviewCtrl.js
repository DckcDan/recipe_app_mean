		...
		RecipeDAO
			...
			.exec(function(err,recipe){
					if(recipe){
						...
						if(err){
							httpHelper.sendJsonResponse(res,500,{"error":"Error in listing reviews "+err});
					
						}else{
							httpHelper.sendJsonResponse(res,404,{"status":"Review not found "+req.params.recipeId});
						}
			...
			.exec(function(err,recipe){
					if(recipe){
						...
						if(review && review.length > 0){
							...
							httpHelper.sendJsonResponse(res,404,{"status":"ReviewId not found "+req.params.reviewId});
						}
 ...
 if(recipeId){
	RecipeDAO
			...
			.exec(function(err, recipe){
				...
				if(err){
					httpHelper.sendJsonResponse(res,500,{"status":"Error getting a recipe - error "+err});	
				}else{
					if(recipe){
...
var addReviewToRecipe = function(req,res,recipe){
	...
	recipe.save(function(err,recipe){
			if(err){
				httpHelper.sendJsonResponse(res,500,{"status":"Error saving a review - error "+err});	
			}else{
				if(recipe){
					var thisReview = recipe.reviews[recipe.reviews.length - 1];
						httpHelper.sendJsonResponse(res,201,thisReview);
					}else{
						httpHelper.sendJsonResponse(res,500,{"status":"Error saving a review  ranting- error "+err});	
					}
			}
